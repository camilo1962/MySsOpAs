...
public class BoardDataDrawer : Editor
{
    ...
    {
        InitializeReordenableList(ref _dataList, "SearchWords", "Searching Words");
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        GameDataInstance.timeInSeconds = EditorGUILayout.FloatField("Máximo tiempo en resolverlo", GameDataInstance.timeInSeconds);
    ...
    private void DrawColumnsRowsInputFields()
    {
        ...
        if ((GameDataInstance.Columns != columsTemp || GameDataInstance.Rows != rowsTemp)
    ...
    {
        var tablesStyle = new GUIStyle("box");
        tablesStyle.padding = new RectOffset(10, 10, 10, 10);
        tablesStyle.margin.left = 32;
        ...
        EditorGUILayout.BeginHorizontal(tablesStyle);
    ...
    private void InitializeReordenableList(ref ReorderableList list, string propertyName, string listLabel)
    {
    ...
    {
        if(GUILayout.Button("Borra Tablero"))
        {
    ...
    private void FillUpWithRandonLettersButton()
    {
        if (GUILayout.Button("Llenado Aleatorio"))
        {
            for (int i = 0; i < GameDataInstance.Columns; i++)
            {
                for (int j = 0; j < GameDataInstance.Rows; j++)
                {
                    ...
                    string letters = "ABCDEFGHIJKLMNÑOPQRSTUVXYZ";
