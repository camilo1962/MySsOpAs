...
public class SearchingWordList : MonoBehaviour
{
    ...
    private bool TryIncreaseColumnNumber()
    {
        ...
        if (_wordsNumber % _wordsNumber > 0)
            _rows++;
    ...
    private Vector3 GetSquareScale(Vector3 defaultScale)
    {
        ...
        var adjuntment = 0.01f;

        while (ShoulScaleDown(finalScale))
        {
            finalScale.x -= adjuntment;
            finalScale.y -= adjuntment;
    ...
    private bool ShoulScaleDown(Vector3 targetScale)
    {
        var squareRect = searchinWordPrefab.GetComponent<RectTransform>();
    ...
    private Vector2 GetFirsSquarePosition()
    {
        ...
        // Certro
        var shifBy = (parentRect.rect.width - (squareSize.x * _columns)) / 2;

        startPosition.x = ((parentRect.rect.width - squareSize.x) / 2) * (-1);
        startPosition.y += shifBy;
